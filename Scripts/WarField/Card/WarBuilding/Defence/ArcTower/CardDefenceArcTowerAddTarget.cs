...
namespace WarField
{
    ...
    //增加1个闪电链攻击目标（2/2）
    public class CardDefenceArcTowerAddTarget : CardEffection<CardDefenceArcTowerAddTarget>
    {
        private DefenceConf _conf;
        public CardDefenceArcTowerAddTarget()
        {
            _category = CD.CardCategory.BUILDING;
            _levelCnt = new[] { 0, 0, 2, 2 };
        }
        ...
        protected override bool OnTakeEffect(CardDefines.CardLevel level)
        {
            ...
            bool improveRet = WarBuildingCtrl.Instance.ApplyImprovement(WE.ImproveSrc.FROMCARD, WE.RaceType.Human, WBD.BuildingMode.DEFENCE, (int)
                HumanDefines.DefenceType.ARCTOWER, "targetNum", value, GD.CalDeltaType.ADD);
            if (improveRet == false)
            {
