...
namespace WarField
{
    ...
    //对建筑的额外伤害伤害增强10%/18% （4/1）   46%/18%    72%
    public class CardDefenceSiegeTowerSiegeDamageUp : CardEffection<CardDefenceSiegeTowerSiegeDamageUp>
    {
        private DefenceConf _conf;
        public CardDefenceSiegeTowerSiegeDamageUp()
        {
        ...
        public override bool CanBeInit()
        {
            if(_conf == null)
                _conf = _conf = (DefenceConf)WarBuildingCtrl.Instance.GetBdConf(WE.RaceType.Human, WBD.BuildingMode.DEFENCE,
                    (int)HumanDefines.DefenceType.SIEGETOWER);
        ...
        protected override bool OnTakeEffect(CardDefines.CardLevel level)
        {
            ...
            bool improveRet = WarBuildingCtrl.Instance.ApplyImprovement(WE.ImproveSrc.FROMCARD, WE.RaceType.Human, WBD.BuildingMode.DEFENCE, (int)
                HumanDefines.DefenceType.SIEGETOWER, "siegeDamageUp", value, GD.CalDeltaType.MUL);
            if (improveRet == false)
            {
