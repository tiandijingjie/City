...
namespace WarField
{
    ...
    //增加8%/18%的爆炸覆盖区域（4/1）      36%/18%              60%
    public class CardDefenceLaserTowerBombRang : CardEffection<CardDefenceLaserTowerBombRang>
    {
        private DefenceConf _conf;
        public CardDefenceLaserTowerBombRang()
        {
        ...
        protected override bool OnTakeEffect(CardDefines.CardLevel level)
        {
            ...
            bool improveRet = WarBuildingCtrl.Instance.ApplyImprovement(WE.ImproveSrc.FROMCARD, WE.RaceType.Human, WBD.BuildingMode.DEFENCE, (int)
                HumanDefines.DefenceType.LASERTOWER, "bombRange", value, GD.CalDeltaType.MUL);
            if (improveRet == false)
            {
