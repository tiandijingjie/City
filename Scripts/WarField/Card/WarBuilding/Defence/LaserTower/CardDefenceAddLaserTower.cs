...
namespace WarField
{
    ...
    //获取激光塔
    public class CardDefenceAddLaserTower : CardEffection<CardDefenceAddLaserTower>
    {
        public CardDefenceAddLaserTower()
        {
        ...
        protected override bool OnTakeEffect(CardDefines.CardLevel level)
        {
            bool improveRet = WarBuildingCtrl.Instance.ApplyImprovement(WE.ImproveSrc.FROMCARD, WE.RaceType.Human, WBD.BuildingMode.DEFENCE, (int)
                HumanDefines.DefenceType.LASERTOWER, "enabled", 0, GD.CalDeltaType.MIN);
            if (improveRet == false)
            {
