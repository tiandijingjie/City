...
namespace WarField
{
    ...
    //获取燃烧塔
    public class CardDefenceAddFlameTower : CardEffection<CardDefenceAddFlameTower>
    {
        public CardDefenceAddFlameTower()
        {
        ...
        protected override bool OnTakeEffect(CardDefines.CardLevel level)
        {
            bool improveRet = WarBuildingCtrl.Instance.ApplyImprovement(WE.ImproveSrc.FROMCARD, WE.RaceType.Human, WBD.BuildingMode.DEFENCE, (int)
                HumanDefines.DefenceType.FLAMETOWER, "enabled", 0, GD.CalDeltaType.MIN);
            if (improveRet == false)
            {
