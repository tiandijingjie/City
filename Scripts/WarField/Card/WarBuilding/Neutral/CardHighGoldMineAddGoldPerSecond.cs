...
namespace WarField
{
    ...
    //x每个高级金矿每秒采矿数量增加10/25 (4/1)  40/25  65
    class CardHighGoldMineAddGoldPerSecond : CardEffection<CardHighGoldMineAddGoldPerSecond>
    {
        public CardHighGoldMineAddGoldPerSecond()
        {
        ...
        public override bool CanBeInit()
        {
            ...
            if (WarBuildingCtrl.Instance.GetBuildingOnField(WE.RaceType.Neutral, WBD.BuildingMode.GOLDMINE, WE.OnGroundMapIndex, out var list) == true)
            {
                ...
                {
                    if (mine.gs_subtype == (int)NeutralDefines.GoldMineType.HIGHLEVL)
                    {
        ...
        protected override bool OnTakeEffect(CardDefines.CardLevel level)
        {
            ...
            bool improveRet = WarBuildingCtrl.Instance.ApplyImprovement(WE.ImproveSrc.FROMCARD, WE.RaceType.Neutral, WBD.BuildingMode.GOLDMINE, (int)
                NeutralDefines.GoldMineType.HIGHLEVL, "goldAddPerSec", value, GD.CalDeltaType.ADD);
            if (improveRet == false)
            {
