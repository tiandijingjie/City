...
namespace WarField
{
    ...
    //增加远程0.24/1点回血（4/1）                                                      0.96/1      2.96
    public class CardRangedHpInc : CardEffection<CardRangedHpInc>
    {
        public CardRangedHpInc()
        {
        ...
        protected override bool OnTakeEffect(CardDefines.CardLevel level)
        {
            ...
            if (level == CD.CardLevel.NORMAL)
                value = 0.24f * Time.fixedDeltaTime;
            else if (level == CD.CardLevel.RARE)
                value = 1f * Time.fixedDeltaTime;
            else
            {
