...
namespace WarField
{
    ...
    // 燃烧持续时间:8s
    // 技能冷却:65s
    public class MagicHeroMeteorStrikeSkill : Skill, MainEffectFinishCb, AreaEffectIndicatorCb, IndividualItemModifyNotifyInf
    {
...
#region private parameters
        private MagicHeroIndividualData.SkillMeteorStrike _curAttribute = null;
        ...
        private (float, float, string, object, BFD.BuffStrategy) _burnObj;
#endregion

#region private parameters' get set
        public override uint gs_skillType
        {
            get { return (uint)MagicHeroIndividualData.IndividualDataType.METEORSTRIKE; }
        }
#endregion
...
#region public functions
        ...
        {
            switch ((MagicHeroIndividualData.SkillMeteorStrike.ParameterType)changeId)
            {
                case MagicHeroIndividualData.SkillMeteorStrike.ParameterType.BURNDURATION:
                case MagicHeroIndividualData.SkillMeteorStrike.ParameterType.BURNDAMAGEPERSEC:
                    _burnObj = (_curAttribute.p_burnDamagePerSec, _curAttribute.p_burnDuration, "MagicHeroMeteorStrikeSkill", (object)this, BFD
                        .BuffStrategy.IGNORE);
                    break;
                default:
                    break;
...
#region private functions
        protected override bool OnActiveSkill()
        {
            if (_curAttribute == null)
                _curAttribute =
                    (MagicHeroIndividualData.SkillMeteorStrike)(_soldier.gs_oriIndividualData as MagicHeroIndividualData).gs_individualItems[
                        (int)MagicHeroIndividualData.IndividualDataType.METEORSTRIKE];
        ...
        private void OnAreaTargetsFound(List<IGridNode> targets)
        {
            for (int i = 0; i < targets.Count; i++)
            {
                ...
                sd.BeAffectedByBuff(BFD.SoldierBuffType.DURATIONDAMAGE, in _burnObj);
            }
        }
